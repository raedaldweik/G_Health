...
agentic stack: Gemini and the Agent Development Kit (a supervisor and five specialists), grounded
...
| Agents | ADK supervisor plus five specialists (data, guidelines, risk, population-health MCP, actions) on Gemini Flash, with a tested fallback chain and a scripted engine that runs the same tools without the language model | Same graph on Cloud Run in me-central1 with sessions in AlloyDB; Agent Engine when available in region |
...
| Simulation | Patient what-if with live re-scoring, attribution, care gaps closed and a Gemini explanation; counterfactual re-scoring of eligible cohorts for five programmes with costs | ML.PREDICT over counterfactual rows |
